...
ll create(lli *v, lli *st, lli ss, lli es, lli si) {
    ...
    lli mid = ss + ((es - ss) / 2);
    st[si] = min(create(v, st, ss, mid, (2 * si) + 1), create(v, st, mid + 1, es, (2*si) + 2));
    return st[si];
}
 
ll getMin(lli *st, lli ss, lli es, lli sr, lli er, lli si) {
    if(er < ss || sr > es) {
        return LONG_MAX;
    }
    ...
    lli mid = ss + (es - ss) / 2;
    return min(getMin(st, ss, mid, sr, er, (si * 2)+ 1), getMin(st, mid + 1, es, sr, er, (si * 2) + 2));
}
 
void update(lli *st, lli ss, lli es, lli i, lli new_val, lli si) {
    if(i <ss || i > es) {
        return;
    }
    if(es == ss) {
        ...
        lli mid = ss + ((es - ss)/ 2);
        if(i >= ss && i <= mid) {
            update(st, ss, mid, i, new_val, (si * 2) + 1);
        } else {
            update(st, mid + 1, es, i, new_val, (si * 2) + 2);
        }
...
int main() {
    ...
    while(q--) {
        ...
        if(t == 1) {
            cin >> k;
            cin >> u;
            update(st, 0, n - 1, k - 1, u,0);
        } else {            
            cin >> a;
            cin >> b;
            ll ans = getMin(st, 0, n - 1, a-1, b-1, 0);
            p_e(ans);
        }
    }
    return 0;
