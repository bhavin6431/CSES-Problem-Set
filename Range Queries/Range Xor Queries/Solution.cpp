...
ll getXor(lli *st, lli ss, lli es, lli sr, lli er, lli si) {
    if(er < ss || sr > es) {
        return 0;
    }
    ...
    lli mid = ss + (es - ss) / 2;
    return getXor(st, ss, mid, sr, er, (si * 2)+ 1) ^ getXor(st, mid + 1, es, sr, er, (si * 2) + 2);
}
    ...
    while(q--) {
       // cin >> t;
        ...
        //} else {            
            cin >> a;
            cin >> b;
            ll ans = getXor(st, 0, n - 1, a-1, b-1, 0);
            p_e(ans);
        //}
    }
    return 0;
