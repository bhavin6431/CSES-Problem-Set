...
ll construct(ll *seg, int ss, int se, int si, int a[]) {
    if(ss == se) {
        seg[si] = a[ss];
        return a[ss];
    }   
    int mid = ss + ((se - ss) / 2);
    seg[si] = min(construct(seg, ss, mid, (si * 2) + 1, a), 
    construct(seg, mid + 1, se, (si * 2) + 2, a));
    return seg[si];
}
 
ll getMin(ll *seg, int ss, int se , int qs, int qe, int si) {
    if(qs <= ss &&  qe >= se) {
        return seg[si];
    }
    if(se < qs || ss > qe) {    
    ...
    int mid = ss + ((se - ss) / 2);
    return min(getMin(seg, ss, mid, qs, qe, (si * 2) + 1), getMin(seg, mid + 1, se, qs, qe, (si * 2) + 2));
}
...
int main() {
    ...
    int a[n];
    read_array(a, n);
    int leaves = ceil(log(n) / log(2));
    int sz = (2 * pow(2, leaves)) - 1;
    ll seg[sz];
 
    construct(seg, 0, n - 1, 0, a);
    for(int i=0;i<sz;i++) {
        debug(seg[i]);
    }
    loop(q) {
        int qs, qe;
        cin >> qs >> qe;
        ll ans = getMin(seg, 0, n - 1, qs - 1 , qe - 1, 0);
        debug(ans);
        p_e(ans);
    }
