...
int main() {
    ...
    fastio();
    read(n);
    map<int, map<int,vector<int>>> arr;
    loop(n){
        int ar;
        cin >> ar;
        arr[ar][0].push_back(i);
        int de;
        cin >> de;
        arr[de][1].push_back(i); 
    } 
    int c = 0;
    int ans = 0;
    map<int,int> room;
    ...
    for(auto it = arr.begin();it != arr.end(); ++it){
        vector<int> arrival = it->second[0];
        loop(arrival.size()){
            int avairoom = room.begin()->first;
            debug(avairoom);
            cust_room[arrival[i]] = avairoom;
            room.erase(avairoom);
            if(room.size() == 0) {
                room.insert({avairoom+1, avairoom+1});
            }
        }
        c+= arrival.size();
        ans = max(ans, c);
        
        vector<int> departure = it->second[1];       
        loop(departure.size()) {
            room.insert({cust_room[departure[i]], cust_room[departure[i]]});
        }
