...
public class Solution {
	...
	public static void minSum(int a[], long sum1, long sum2, int index, int n) {
		if (index > n - 1) {
			ans = Math.min(ans, Math.abs(sum1 - sum2));
			return;
		}
		minSum(a, sum1 + a[index], sum2, index + 1, n);
		minSum(a, sum1, sum2 + a[index], index + 1, n);
	}
