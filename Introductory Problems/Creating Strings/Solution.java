...
public class Solution {
	...
	static final ArrayList<String> moves = new ArrayList<>();
	...
	public static void gen(char[] s, String prefix, Set<String> ans) {
		...
		for (int i = prefix.length(); i < s.length; i++) {
			char[] c1 = Arrays.copyOf(s, s.length);
			if (i != prefix.length()) {
				char t = c1[i];
				c1[i] = c1[prefix.length()];
				c1[prefix.length()] = t;
			}
			gen(c1, prefix + s[i], ans);
		}
