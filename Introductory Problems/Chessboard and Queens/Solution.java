...
public class Solution {
	...
	public static void main(String args[]) throws IOException { // BufferedReader
		...
		boolean d1[] = new boolean[16];
		boolean d2[] = new boolean[16];
		boolean r[][] = new boolean[8][8];
 
		for (int i = 0; i < 8; i++) {
			char[] c = r_s(br).toCharArray();
			for (int j = 0; j < c.length; j++) {
				r[i][j] = (c[j] == '*');
			}
		}
 
		search(0, col, d1, d2, r);
		pr_o_nl(ways);
	}
 
	public static void search(int r, boolean col[], boolean d1[], boolean d2[], boolean[][] re) {
		if (r == 8) {
			ways++;
			return;
		}
		for (int c = 0; c < 8; c++) {
			if (col[c] || d1[r + c] || d2[r - c + 7] || re[r][c]) {
				continue;
			}
			col[c] = d1[r + c] = d2[r - c + 7] = re[r][c] = true;
			search(r + 1, col, d1, d2, re);
			col[c] = d1[r + c] = d2[r - c + 7] = re[r][c] = false;
		}
	}
